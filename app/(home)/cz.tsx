...
const Children = () => {
       ...
       const fetchAnimals = async () => {
           ...
           try {
               ...
               if (!snapshot.empty) {
                   ...
                   // Filter animals in Children location - handle both array and string formats
                   const childrenAnimals = allAnimals.filter(animal => {
                       if (!animal.location) return false;

                       // Handle array format
                       if (Array.isArray(animal.location)) {
                           return animal.location.some(loc =>
                               loc === 'Children' || loc.toLowerCase() === 'children'
                           );
                       }

                       // Handle string format
                       return animal.location === 'Children' || animal.location === 'children';
                   });

                   // Sort by name
                   const sortedAnimals = childrenAnimals.sort((a, b) =>
   ...
   const renderAnimalCard = ({ item }: { item: Animal }) => (
       ...
       >
           <Card.Section
               imageSource={item.photo ? { uri: item.photo } : require('../../assets/images/home/children.png')}
           ...
           <FlatList
               ...
               ListEmptyComponent={
                   <ThemedView style={styles.emptyContainer}>
                       <ThemedText style={styles.emptyText}>No animals found in Children's Zoo.</ThemedText>
                   </ThemedView>
               }
           />
       </ThemedView>
